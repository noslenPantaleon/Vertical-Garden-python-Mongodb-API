...
def on_connect(client, flags, rc, properties):
    client.subscribe("microstation")  # subscribing mqtt topic
...
async def mqtt_message(client, topic, payload, qos, properties):
    ...
    for key in convert_mqtt_json.keys():  # create new dictionary to changes keys
        ...
        rack_data["ambient_temperature_1"] = ambient_temperature_1_recieve

        ambient_temperature_2_recieve = val.get('AT2')
        rack_data["ambient_temperature_2"] = ambient_temperature_2_recieve

        ambient_humydity_1_recieve = val.get('AH1')
        rack_data["ambient_humidity_1"] = ambient_humydity_1_recieve

        ambient_humydity_2_recieve = val.get('AH2')
        rack_data["ambient_humidity_2"] = ambient_humydity_2_recieve

        soil_humydity_1a_recieve = val.get('SH1a')
        rack_data["soil_humydity_1a"] = soil_humydity_1a_recieve

        soil_humydity_1b_recieve = val.get('SH1b')
        rack_data["soil_humydity_1b"] = soil_humydity_1b_recieve

        soil_humydity_2a_recieve = val.get('SH2a')
        rack_data["soil_humydity_2a"] = soil_humydity_2a_recieve

        soil_humydity_2b_recieve = val.get('SH2b')
        rack_data["soil_humydity_2b"] = soil_humydity_2b_recieve

        water_temperature_micros_recieve = val.get('WTM')
        rack_data["water_temperature_microgreens"] = water_temperature_micros_recieve

        water_temperature_hydroponics_recieve = val.get('WTH')
        rack_data["water_temperature_hydroponics"] = water_temperature_hydroponics_recieve

        water_level_microgreens_recieve = val.get('WLT1')
        rack_data["water_level_microgreens"] = water_level_microgreens_recieve

        water_level_hydroponics_recieve = val.get('WLT2')
        rack_data["water_level_hydroponics"] = water_level_hydroponics_recieve

        water_level_tray_1_recieve = val.get('WLT1')
        rack_data["water_level_tray_1 "] = water_level_tray_1_recieve
